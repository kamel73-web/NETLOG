...
export default function App() {
  ...
  const incrementCounter = (
    key: "offres" | "missions" | "factures" | "ldv" | "devis",
  ) => {
    setCounters((prev) => {
  ...
  const [darkMode, setDarkMode] = useState<boolean>(
    () => localStorage.getItem("netlog_theme") === "dark",
  );
  useEffect(() => {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      showToast(
        lang === "ar" ? "✅ تم استعادة الاتصال" : "✅ Connexion rétablie",
        "success",
      );
    };
    const handleOffline = () => {
  ...
  const [activeLegalModal, setActiveLegalModal] = useState<
    "about" | "cgu" | "privacy" | "contact" | null
  >(null);
  ...
  const translateProfile = (p: ProfileType | string) => {
    if (p === ProfileType.DonneurOrdre || p === "Donneur d'ordre")
      return t("profilDonneur");
    if (p === ProfileType.Transporteur || p === "Transporteur")
      return t("profilTransporteur");
    if (p === ProfileType.Commissionnaire || p === "Commissionnaire")
      return t("profilCommissionnaire");
    if (p === ProfileType.Manutentionnaire || p === "Manutentionnaire")
      return t("profilManutentionnaire");
    if (p === ProfileType.Stockage || p === "Espace de stockage")
      return t("profilStockage");
    return String(p);
  };

  const translateStatus = (s: OffreStatus | string) => {
    if (s === OffreStatus.Publie || s === "Publié") return t("statutPublie");
    if (s === OffreStatus.Attribue || s === "Attribué")
      return t("statutAttribue");
    if (s === OffreStatus.Charge || s === "Chargé / En cours")
      return t("statutCharge");
    if (s === OffreStatus.Decharge || s === "Déchargé")
      return t("statutDecharge");
    if (s === OffreStatus.Valide || s === "Validé / Clôturé")
      return t("statutValide");
    return String(s);
  };
  ...
  const [selectedDetailOffre, setSelectedDetailOffre] =
    useState<OffreFret | null>(null);
  ...
  const [subSelectedPlan, setSubSelectedPlan] = useState<
    "free" | "premium" | "gold"
  >("premium");
  const [subSelectedDuration, setSubSelectedDuration] = useState<
    1 | 3 | 6 | 12
  >(12);
  const [activeSubTabMenu, setActiveSubTabMenu] = useState<
    "profiles" | "subscription" | "legals"
  >("profiles");
  ...
  const [pubMarchandise, setPubMarchandise] = useState(
  ...
  const [sortBy, setSortBy] = useState<"recent" | "price" | "proximity">(
    "recent",
  );
  ...
  const [regProfil, setRegProfil] = useState<ProfileType>(
    ProfileType.DonneurOrdre,
  );
  ...
  const [regTransTypeEntite, setRegTransTypeEntite] = useState(
  ...
  const [recrutProfilType, setRecrutProfilType] = useState<ProfileType>(
  ...
  const [regManTypesEngins, setRegManTypesEngins] = useState<string[]>([
    "Chariot élévateur",
  ]);
  ...
  const [transporteurSubTab, setTransporteurSubTab] = useState<
    "flotte" | "offres" | "missions" | "finances" | "profil"
  >("flotte");
  const [transOffresSubTab, setTransOffresSubTab] = useState<
    "compatibles" | "toutes" | "favoris"
  >("compatibles");
  const [transMissionsSubTab, setTransMissionsSubTab] = useState<
    "en_cours" | "terminees" | "annulees"
  >("en_cours");
  const [transFinancesPeriod, setTransFinancesPeriod] = useState<
    "mois" | "dernier" | "3mois" | "6mois" | "annee"
  >("mois");
  ...
  const [moyenDispo, setMoyenDispo] = useState<
    "Disponible" | "Occupé" | "En maintenance"
  >("Disponible");

  // Formulaire Soumission offre (Transporteur)
  const [selectedOffreForBid, setSelectedOffreForBid] =
    useState<OffreFret | null>(null);
  ...
  const handleApproveUser = async (userId: string) => {
    ...
    triggerSystemLog(
      "Compte d'acteur validé KYC administrativement avec succès. Email de notification transmis.",
      "success",
    );
  };
  ...
  const handleSuspendUser = async (userId: string) => {
    ...
    const previous = users;
    const updated = users.map((u) =>
      u.id === userId ? { ...u, status: "suspendu" as const } : u,
    );
    setUsers(updated);
    ...
    triggerSystemLog(
      "Compte d'acteur suspendu immédiatement pour motif administratif.",
      "info",
    );
  };
  ...
  const handleAddWithdrawal = (mont: number, method: string) => {
    ...
    triggerSystemLog(
      `Virement de commission de ${mont.toLocaleString()} DA demandé avec succès. En cours de traitement par NETLOG.`,
      "success",
    );
  };
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (!profileRow) {
      triggerSystemLog(
        "Connecté, mais impossible de charger votre profil. Réessayez.",
        "danger",
      );
      return;
    }
    ...
    if (uStatus === "en_attente") {
      triggerSystemLog(
        "Votre compte est en cours de validation. Merci de patienter.",
        "danger",
      );
      await signOut();
      return;
    ...
    if (uStatus === "suspendu") {
      triggerSystemLog(
        "Votre compte a été suspendu. Contactez NETLOG.",
        "danger",
      );
      await signOut();
      return;
    ...
    try {
      ...
      triggerSystemLog(
        "Connexion OK mais échec du chargement des données.",
        "danger",
      );
    }
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!telRegex.test(cleanedTel)) {
      triggerSystemLog(
        "Format de téléphone incorrect (ex: 0555123456 pour 10 chiffres)",
        "danger",
      );
      return;
    }
    ...
    if (regProfil === ProfileType.Admin) {
      triggerSystemLog(
        "La création de compte administrateur n'est pas disponible par inscription publique.",
        "danger",
      );
      return;
    }
    ...
    if (regProfil === ProfileType.DonneurOrdre) {
      ...
      if (regDoTypeEntite !== "Particulier") {
        if (!regDoRaisonSociale.trim()) {
          triggerSystemLog(
            "Raison sociale obligatoire pour les professionnels",
            "danger",
          );
          return;
        }
      ...
      if (!regTransRC.trim()) {
        triggerSystemLog(
          "Numéro d'inscription RC obligatoire pour les transporteurs",
          "danger",
        );
        return;
      }
      ...
      if (!regManRaisonSociale.trim()) {
        triggerSystemLog(
          "Raison sociale obligatoire pour les manutentionnaires",
          "danger",
        );
        return;
      }
      if (!regManRC.trim()) {
        triggerSystemLog(
          "Numéro d'inscription RC obligatoire pour les manutentionnaires",
          "danger",
        );
        return;
      }
      ...
      if (!regManRaisonSociale.trim()) {
        triggerSystemLog(
          "Raison sociale obligatoire pour les commissionnaires",
          "danger",
        );
        return;
      }
      if (!regManRC.trim()) {
        triggerSystemLog(
          "Numéro d'inscription RC obligatoire pour les commissionnaires",
          "danger",
        );
        return;
      }
      ...
      if (!regManRaisonSociale.trim()) {
        triggerSystemLog(
          "Raison sociale obligatoire pour l'espace de stockage",
          "danger",
        );
        return;
      }
      if (!regManRC.trim()) {
        triggerSystemLog(
          "Numéro d'inscription RC obligatoire pour l'espace de stockage",
          "danger",
        );
        return;
      }
    ...
    loadProfiles()
      ...
      .catch((err) => {
        console.error(
          "Erreur rafraîchissement profils après inscription:",
          err,
        );
      });

    triggerSystemLog(
      "✅ Compte créé ! Votre inscription est en cours de validation par l'équipe NETLOG.",
      "success",
    );
  };
  ...
  const handleCreateOffre = async (e: React.FormEvent) => {
    ...
    try {
      const inserted = await createFreightOffer({
        wilayaDepart: Number(wilayaDepartObj.code),
        wilayaArrivee: Number(wilayaArriveeObj.code),
        ...
        poidsKg: formPoids ? Number(formPoids) * 1000 : undefined,
        ...
        typeMoyenExige: formMoyen || undefined,
        nombreVoyages: formVoyages ? Number(formVoyages) : 1,
      });
      insertedId = inserted.id;
    } catch (err: any) {
      triggerSystemLog(
        `Échec de la publication de l'offre : ${err.message}`,
        "danger",
      );
      return;
    }
    ...
    saveState(undefined, undefined, [...offres, newOffre]);
    triggerSystemLog(
      "Demande d'offre de fret publiée sur la bourse !",
      "success",
    );
  ...
  const handleAcceptBid = (bidId: string) => {
    ...
    const updatedProps = propositions.map((p) => {
      if (p.offreId === targetOffre.id) {
        return {
          ...p,
          status: p.id === bidId ? ("Accepté" as const) : ("Rejeté" as const),
        };
      }
      return p;
    });
    ...
    triggerSystemLog(
      `Offre attribuée à ${bid.transporteurRaisonSociale}. Contrat DO-BDF validé d'office !`,
      "success",
    );
  };
  ...
  const initiateBid = (offre: OffreFret) => {
    ...
    const compatibleMoyens = moyens.filter(
  ...
  const handlePublishBid = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Number.isFinite(offerId)) {
        throw new Error("Identifiant de l'offre invalide pour Supabase.");
      }

      const vehicleId = Number(linkedMoyen.id);

      if (!Number.isFinite(vehicleId)) {
        throw new Error("Identifiant du véhicule invalide pour Supabase.");
      }
      ...
      console.error("[NETLOG] Erreur soumission proposition :", error);
  ...
  const handleUpdateStatus = (offreId: string, nextStatus: OffreStatus) => {
    ...
    if (nextStatus === OffreStatus.Charge) {
      triggerSystemLog(
        "La cargaison est officiellement à bord. Lettre de voiture visée !",
        "success",
      );
    } else if (nextStatus === OffreStatus.Decharge) {
      triggerSystemLog(
        "Déchargement validé au quai d'arrivée ! En attente du code de livraison.",
        "info",
      );
    }
  };
  ...
  const filteredBourseOffres = offres.filter((offre) => {
    ...
    if (
      filterDepart &&
      filterDepart !== "Tous" &&
      !offre.depart.toLowerCase().includes(filterDepart.toLowerCase())
    )
      return false;
    if (
      filterArrivee &&
      filterArrivee !== "Tous" &&
      !offre.arrivee.toLowerCase().includes(filterArrivee.toLowerCase())
    )
      return false;
    if (
      filterMarchandise &&
      !offre.marchandise.toLowerCase().includes(filterMarchandise.toLowerCase())
    )
      return false;
    if (filterMoyen !== "Tous" && offre.moyenExige !== filterMoyen)
      return false;
  ...
  const getDoMetrics = () => {
    ...
    const linkedFactures = factures.filter(
      (f) => f.donneurId === currentUser?.id,
    );

    const facturesRecues = linkedFactures.reduce(
      (acc, f) => acc + f.montant,
      0,
    );
    const montantsRegles = linkedFactures
      .filter((f) => f.status === FactureStatus.Reglee)
      .reduce((acc, f) => acc + f.montant, 0);
    const montantsNonRegles = linkedFactures
      .filter((f) => f.status !== FactureStatus.Reglee)
      .reduce((acc, f) => acc + f.montant, 0);
  ...
  const getTransMetrics = () => {
    const transFactures = factures.filter(
      (f) => f.transporteurId === currentUser?.id,
    );
    const transPropositions = propositions.filter(
      (p) => p.transporteurId === currentUser?.id,
    );
    const transMoyensCount = moyens.filter(
      (m) => m.transporteurId === currentUser?.id,
    ).length;

    const caRealise = transFactures
      .filter((f) => f.status === FactureStatus.Reglee)
      .reduce((acc, f) => acc + f.montant, 0);
    const facturesTransmises = transFactures
      .filter((f) => f.status === FactureStatus.Transmise)
      .reduce((acc, f) => acc + f.montant, 0);
    const creances = transFactures
      .filter((f) => f.status !== FactureStatus.Reglee)
      .reduce((acc, f) => acc + f.montant, 0);
    ...
    return {
      ...
      servicesConfirmes: offres.filter((o) => o.status === OffreStatus.Attribue)
        .length,
      caRealise,
      facturesTransmises,
      creances,
  ...
  const getEstimatedDistance = (from: string, to: string) => {
    ...
    if (
      (fLower === "alger" && tLower === "sétif") ||
      (fLower === "sétif" && tLower === "alger")
    )
      return "300 km";
    if (
      (fLower === "oran" && tLower === "adrar") ||
      (fLower === "adrar" && tLower === "oran")
    )
      return "1200 km";
    if (
      (fLower === "béjaïa" && tLower === "alger") ||
      (fLower === "alger" && tLower === "béjaïa")
    )
      return "220 km";
    if (
      (fLower === "blida" && tLower === "constantine") ||
      (fLower === "constantine" && tLower === "blida")
    )
      return "400 km";
    return "~350 km";
  };
  ...
  const renderDetailOffreScreen = (offre: OffreFret) => {
    ...
    const myBid = propositions.find(
      (p) => p.offreId === offre.id && p.transporteurId === currentUser?.id,
    );
    ...
    if (offre.status === OffreStatus.Attribue) {
      ...
      offre.status === OffreStatus.Decharge ||
      offre.status === OffreStatus.Valide
    ) {
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-3xs">
          ...
          >
            ...
            {lang === "ar"
              ? "العودة إلى البورصة"
              : "Retour à la Bourse d'offres"}
          </button>

          <span className="text-slate-400 font-mono text-[11px] font-bold">
            STATUT:{" "}
            <strong className="uppercase text-[#1D9E75]">{offre.status}</strong>
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
              <div className="flex flex-wrap justify-between items-center gap-3">
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1 text-[11px] font-black rounded-full border ${statusClass}`}
                  >
                    🔵 {displayStatusLabel}
                  </span>
              ...
              <div>
                <h1 className="text-xl font-bold font-sans text-slate-900 tracking-tight">
                  Acheminement de{" "}
                  {translateMarchandise(offre.marchandise, lang)} ({offre.poids}{" "}
                  tonnes)
                </h1>
                <p className="text-[11px] text-slate-400 mt-1">
                  📅 Publié le{" "}
                  {new Date(
                    offre.dateCreation || "2026-05-25",
                  ).toLocaleDateString(lang === "ar" ? "ar-DZ" : "fr-FR")}{" "}
                  par{" "}
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
              <span className="text-[10px] text-slate-400 uppercase font-black tracking-wide block">
                🌍 Trajet logistique
              </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-11 gap-4 items-center p-5 bg-gradient-to-br from-slate-50 to-emerald-50/10 rounded-2xl border border-slate-100">
                <div className="md:col-span-4 space-y-1">
                  <span className="text-[9px] uppercase font-bold text-[#1D9E75] tracking-widest block font-mono">
                    📍 DEPART (WILAYA)
                  </span>
                  <p className="font-extrabold text-sm text-slate-800">
                    {translateCity(offre.depart, lang).toUpperCase()}
                  </p>
                  <p className="text-[11px] text-slate-500 font-medium">
                    {offre.departDetails || "Dépôt d'Enlèvement central"}
                  </p>
                </div>
                ...
                <div className="md:col-span-3 flex flex-col items-center justify-center py-2 md:py-0">
                  ...
                  <span className="text-[#1D9E75] font-black text-lg h-6">
                    ➔➔➔
                  </span>
                </div>

                <div className="md:col-span-4 space-y-1">
                  <span className="text-[9px] uppercase font-bold text-rose-500 tracking-widest block font-mono">
                    📍 ARRIVEE (WILAYA)
                  </span>
                  <p className="font-extrabold text-sm text-slate-800">
                    {translateCity(offre.arrivee, lang).toUpperCase()}
                  </p>
                  <p className="text-[11px] text-slate-500 font-medium">
                    {offre.arriveeDetails ||
                      "Dépôt de livraison de destination"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4.5 text-xs text-slate-700">
                <div className="space-y-0.5 border-b pb-2 sm:border-0 sm:pb-0">
                  ...
                  <span className="font-extrabold text-slate-800">
                    {translateMarchandise(offre.marchandise, lang)}
                  </span>
                </div>
                <div className="space-y-0.5 border-b pb-2 sm:border-0 sm:pb-0">
                  <span className="block text-[10px] text-slate-400 font-bold uppercase">
                  ...
                  <span className="font-mono font-black text-[#1D9E75]">
                    {offre.poids} tonnes
                  </span>
                </div>
                ...
                <div className="space-y-0.5 pt-2">
                  ...
                  <span className="font-mono font-bold">
                    {offre.longueurExigee || 12.0} mètres
                  </span>
                </div>
                <div className="space-y-0.5 pt-2">
                  <span className="block text-[10px] text-slate-400 font-bold uppercase">
                  ...
                  <span className="font-extrabold text-slate-800">
                    {offre.nombreVoyages} voyage
                    {offre.nombreVoyages > 1 ? "s" : ""}
                  </span>
                </div>
                <div className="space-y-0.5 pt-2">
                  <span className="block text-[10px] text-slate-400 font-bold uppercase">
            ...
            {offre.status !== OffreStatus.Publie && (
              <div className="bg-emerald-50/80 border border-emerald-200/50 p-6 rounded-3xl space-y-4">
                <span className="text-[10px] text-emerald-800 uppercase font-black tracking-wide block">
                  📞 Informations de liaison (Contact direct)
                </span>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-4 rounded-xl border border-emerald-200/10">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400">
                      DO de transport d'offres permanent certifié par NETLOG
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() =>
                        triggerSystemLog(
                          `Appel sortant simulé vers le donneur d'ordre (${offre.donneurRaisonSociale}) au +213(0)23102040...`,
                          "success",
                        )
                      }
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
                <div className="border-b pb-3 flex justify-between items-center flex-wrap gap-2">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-400">
                      Comparez et sélectionnez le prestataire pour lui confier
                      cette mission immédiatement.
                    </p>
                  </div>
                </div>
                ...
                {linkedBids.length === 0 ? (
                  <div className="p-8 text-center text-slate-400 text-xs italic">
                    Aucune proposition reçue pour le moment de la part de notre
                    réseau de transporteurs.
                  </div>
                ) : (
                  <div className="space-y-4">
                    {(() => {
                      const cheapest = [...linkedBids].sort(
                        (a, b) => a.prixPropose - b.prixPropose,
                      )[0];

                      return linkedBids.map((bid) => {
                        const score = 4.8;
                        const isCheapest =
                          bid.id === cheapest.id && linkedBids.length > 1;

                        return (
                          <div
                            key={bid.id}
                            className={`p-4 rounded-2xl border transition-all flex flex-col md:flex-row justify-between gap-4 ${
                              offre.status === OffreStatus.Attribue &&
                              bid.status === "Accepté"
                          ...
                          >
                              ...
                              <div className="text-xs text-slate-600 space-y-1 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 font-mono">
                                <p>
                                  ...
                                  <strong className="text-slate-800">
                                    {moyens.find((m) => m.id === bid.moyenId)
                                      ?.marque || "Camion plateau"}
                                  </strong>
                                </p>
                                {bid.commentaire && (
                                  <p>💬 Note : "{bid.commentaire}"</p>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex md:flex-col items-end justify-between md:justify-center gap-2 border-t md:border-t-0 pt-3 md:pt-0 shrink-0">
                              <div className="text-right">
                                ...
                                <span className="text-base font-black text-[#1D9E75] font-mono">
                                  {bid.prixPropose.toLocaleString()} DZD
                                </span>
                              </div>

                              <div className="flex gap-1.5">
                                {bid.status === "En attente" &&
                                offre.status === OffreStatus.Publie ? (
                                  <button
                                    type="button"
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#101F1A] p-6 rounded-3xl text-white shadow-md space-y-4">
              <span className="text-[9px] text-[#1D9E75] uppercase font-black tracking-widest block font-mono">
                💰 BUDGET LOGISTIQUE CIBLE
              </span>
                ...
                {offre.prixFixe ? (
                  ...
                  <>
                    ...
                    <p className="text-[11px] text-amber-200/70 font-semibold">
                      Proposez votre meilleure tarification à de l'arbitrage
                      direct.
                    </p>
                  </>
                )}
              ...
              <div className="border-t border-[#1D9E75]/20 pt-3 text-[11px] text-slate-300 space-y-2">
                <p>
                  💸 Paiement garanti par la plateforme NETLOG après
                  confirmation asymétrique de livraison.
                </p>
                <p>
                  📉 Commissions incluses : 200 DA pour le courtage de
                  transaction.
                </p>
              </div>
            </div>
                    ...
                    {offre.status === OffreStatus.Publie ? (
                      myBid ? (
                        <div className="space-y-3 text-xs">
                          <div className="bg-emerald-50 text-emerald-800 p-3.5 rounded-xl border border-emerald-200 leading-relaxed">
                            ...
                            <p className="mt-1 font-mono">
                              ...
                              <strong className="font-black">
                                {myBid.prixPropose.toLocaleString()} DZD
                              </strong>
                            </p>
                            <p className="text-[10px] text-slate-400 mt-1">
                              En attente de réponse ou sélection par le donneur
                              d'ordre.
                            </p>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const updatedBids = propositions.filter(
                                (p) => p.id !== myBid.id,
                              );
                              saveState(
                                undefined,
                              ...
                              triggerSystemLog(
                                "Votre proposition a été retirée.",
                                "info",
                              );
                            }}
                      ...
                      <div className="bg-slate-50 rounded-xl p-4 border text-[11px] leading-relaxed text-slate-500">
                        <p className="font-bold text-slate-700">
                          🔒 Statut de la Bourse suspendu
                        </p>
                        <p className="mt-1">
                          Cette offre est déjà attribuée ou active en cours de
                          transport sur la plateforme.
                        </p>
                      </div>
                    )}
                  </>
                ...
                {isOwner && (
                  <div className="space-y-3">
                    <p className="text-[11px] italic text-slate-400">
                      Vous êtes le Donneur d'Ordre propriétaire de cette offre.
                    </p>
                ...
                {!currentUser && (
                  <div className="space-y-3 text-center">
                    <p className="text-[11px] text-slate-400 font-semibold">
                      Identifiez-vous comme transporteur routier certifié pour
                      pouvoir chiffrer cette expédition.
                    </p>
                    <button
                      type="button"
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <div className="text-center space-y-1.5 animate-pulse">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">
              {lang === "ar"
                ? "بورصة الشحن بالجزائر"
                : "Bourse de Fret Algérie"}
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] dark:bg-slate-950 flex flex-col text-[#1A1A2E] dark:text-slate-100 pb-[80px] transition-colors duration-300">
      ...
      <div className="fixed bottom-20 right-4 left-4 sm:left-auto sm:w-96 z-50 flex flex-col gap-2.5 pointer-events-none">
        {toasts.map((t) => {
          ...
          return (
            ...
            >
              <span className="text-sm shrink-0" style={{ color: "#ffffff" }}>
                {icon}
              </span>
              <span
              ...
              <button
                className="text-[10px] opacity-75 hover:opacity-100 font-extrabold uppercase ml-2 select-none"
                style={{ color: "#ffffff" }}
              >
                ✕
              </button>
        ...
        <div className="flex items-center gap-3">
          ...
          {currentUser ? (
            <div className="flex items-center gap-2.5">
              ...
              {(() => {
                let colorBg =
                  "bg-blue-100 text-[#0C447C] font-black border border-blue-200";
                let label = "DO";
                if (currentUser.profil === ProfileType.Transporteur) {
                  colorBg =
                    "bg-emerald-100 text-[#085041] font-black border border-emerald-200";
                  label = "TR";
                  ...
                  colorBg =
                    "bg-orange-100 text-[#993C1D] font-black border border-orange-200";
                  label = "BVF";
                } else if (currentUser.profil === ProfileType.Admin) {
                  colorBg =
                    "bg-purple-100 text-[#3B1E7B] font-black border border-purple-200";
                  label = "ADM";
                }
                ...
                return (
                  <div className="flex items-center gap-2 border-l pl-2.5">
                    <button
                      onClick={() => {
                        if (currentUser.profil === ProfileType.DonneurOrdre) {
                          ...
                          currentUser.profil === ProfileType.Transporteur
                        ) {
                          setCurrentTab("transporteur");
                        } else if (
                          currentUser.profil === ProfileType.Chauffeur
                        ) {
                          setCurrentTab("chauffeur");
                        } else if (
                          currentUser.profil === ProfileType.Commercial
                        ) {
                          setCurrentTab("commercial");
                        } else if (currentUser.profil === ProfileType.Admin) {
                          setCurrentTab("admin");
                    ...
                    >
                      {currentUser.nom.substring(0, 1)}
                      {currentUser.prenom.substring(0, 1)}
                    </button>
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder={
                  lang === "ar"
                    ? "ابحث عن عروض (مسار، بضاعة)، ناقلين (ولاية) أو مهام..."
                    : "Rechercher des offres (trajet, marchandise), transporteurs (raison sociale, wilaya) ou missions..."
                }
            ...
            {globalSearchQuery.trim().length > 0 &&
              (() => {
                ...
                const matchedOffres = offres.filter(
                  (o) =>
                        ...
                        {matchedOffres.length > 0 && (
                          <div className="space-y-2">
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-[#1D9E75] flex items-center gap-1.5">
                              📦 OFFRES DE FRET EN DIRECT (
                              {matchedOffres.length})
                            </h4>
                              ...
                              {matchedOffres.map((o) => (
                                <div
                                  ...
                                  onClick={() => {
                                    ...
                                    setGlobalSearchOpen(false);
                                    showToast(
                                      `Affichage détaillé de l'offre ${o.id}`,
                                      "info",
                                    );
                                  }}
                          ...
                          <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {matchedTransporteurs.map((usr) => (
                                ...
                                >
                                  <div>
                                    <span className="font-extrabold text-[#1A1A2E] dark:text-white block">
                                      {usr.raisonSociale ||
                                        `${usr.prenom} ${usr.nom}`}
                                    </span>
                                    <span className="text-[10px] text-slate-500 font-medium block">
                                      📍 Wilaya : {usr.wilaya || "Alger"} ·{" "}
                                      {usr.tel}
                                    </span>
                                  </div>
                        ...
                        {matchedMissions.length > 0 && (
                          <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-amber-600 flex items-center gap-1.5">
                              ⚙️ EN COURS / HISTORIQUE MISSIONS (
                              ...
                              {matchedMissions.map((m) => (
                                ...
                                >
                                  <div className="flex justify-between items-center mb-1">
                                    ...
                                    <span className="text-[10px] font-bold text-amber-600 font-mono tracking-wider">
                                      {m.id}
                                    </span>
                                  </div>
                                  <div className="text-[10px] text-slate-500 flex justify-between">
                                    <span>
                                      Client:{" "}
                                      {m.donneurRaisonSociale ||
                                        "NETLOG Client"}
                                    </span>
                                    <span className="capitalize">
                                      {m.moyenMarque || "Tracteur"}
                                    </span>
                                  </div>
                                </div>
                              ))}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {!currentUser && !guestMode ? (
          /* =========================================================================
             ...
             ========================================================================= */
          <div
            className="max-w-4xl mx-auto my-2 md:my-8 bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden font-sans"
            id="netlog-auth-portail"
          >
            <div
              ...
              <div className="md:col-span-5 bg-[#085041] p-6 text-white flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                  <span className="text-[10px] font-extrabold uppercase bg-emerald-500/20 text-emerald-300 px-2 text-center py-1 rounded-full border border-emerald-400/20">
                    {lang === "ar"
                      ? "بورصة الشحن الرسمية"
                      : "Bourse de Fret Officielle"}
                  </span>

                  <h2 className="text-xl md:text-2xl font-black leading-tight tracking-tight mt-2">
                    {lang === "ar"
                      ? "NETLOG – ملتقى نقل الشحن في الجزائر"
                      : "NETLOG – Le carrefour du transport de fret en Algérie"}
                  </h2>
                ...
                <div className="relative z-10 pt-8 space-y-3">
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <div>
                      <strong className="text-white block font-extrabold">
                        {lang === "ar"
                          ? "إدارة مبسطة لـ KYC"
                          : "Gestion KYC Simplifiée"}
                      </strong>
                      <span className="text-[10px] text-emerald-100 text-slate-100 font-medium">
                        {lang === "ar"
                          ? "التسجيل عبر التحقق من السجل التجاري (RC) بشكل رسمي."
                          : "Inscription par validation de Registre de Commerce (RC)."}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-emerald-100 text-slate-100 font-medium">
                        {lang === "ar"
                          ? "تأكيد واستلام الشحنة برمز سري فريد OTP."
                          : "Confirmation de décharge par code secret unique."}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <div>
                      <strong className="text-white block font-extrabold">
                        {lang === "ar"
                          ? "بدون عمولات مخفية"
                          : "Zéro Commission Masquée"}
                      </strong>
                      <span className="text-[10px] text-emerald-100 text-slate-100 font-medium">
                        {lang === "ar"
                          ? "عروض الأسعار والدفع يتم مباشرة وبشفافية بين الأطراف وبلا وسطاء."
                          : "Les cotations se font en toute franchise et en direct."}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative z-10 pt-8 border-t border-emerald-800 flex justify-between items-center text-[10px] text-emerald-200">
                  ...
                  <span className="font-mono">
                    {lang === "ar" ? "تطبيق الويب متوفر" : "PWA Disponible"}
                  </span>
                </div>
              </div>
              ...
              <div className="md:col-span-7 p-6 sm:p-8 flex flex-col justify-between">
                    ...
                    <div className="space-y-4 animate-fade-in">
                      <div>
                        <h3 className="text-lg font-black text-slate-800">
                          {lang === "ar"
                            ? "الولوج إلى مساحة NETLOG"
                            : "Accéder à l'Espace NETLOG"}
                        </h3>
                        <p className="text-[11px] text-slate-400 font-medium">
                          {lang === "ar"
                            ? "يرجى إدخال بيانات الاعتماد لإدارة نشاط الشحن البري الخاص بك"
                            : "Indiquez vos identifiants pour administrer votre activité de fret"}
                        </p>
                      </div>
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <button
                              ...
                              onClick={() =>
                                triggerSystemLog(
                                  lang === "ar"
                                    ? "لحسابات التجربة الافتراضية، استخدم كلمة المرور 'Test@2025'"
                                    : "Pour les comptes d'essai de démo, utilisez le mot de passe 'Test@2025'",
                                  "info",
                                )
                              }
                    ...
                    <div className="space-y-4 animate-fade-in">
                      ...
                      {regStep === 1 && (
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div
                              onClick={() =>
                                setRegProfil(ProfileType.DonneurOrdre)
                              }
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <strong className="text-xs text-slate-850 block font-extrabold">
                                  {lang === "ar"
                                    ? "حساب طالب شحن (آمر صرف Fret)"
                                    : "Espace Donneur d'Ordre (DO)"}
                                </strong>
                            ...
                            <div
                              onClick={() =>
                                setRegProfil(ProfileType.Transporteur)
                              }
                              className={`p-3 rounded-2xl border-2 transition-all flex items-center gap-3 cursor-pointer ${
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <strong className="text-xs text-slate-850 block font-extrabold">
                                  {lang === "ar"
                                    ? "ناقل بضائع بري / مقدم خدمات لوجستية"
                            ...
                            <div
                              onClick={() =>
                                setRegProfil(ProfileType.Commercial)
                              }
                              className={`p-3 rounded-2xl border-2 transition-all flex items-center gap-3 cursor-pointer ${
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <strong className="text-xs text-slate-850 block font-extrabold">
                                  {lang === "ar"
                                    ? "وكيل تجاري مستقل في BVF"
                                    : "Commercial indépendant de BVF"}
                                </strong>
                                <span className="text-[10px] text-slate-500 font-semibold block">
                                  {lang === "ar"
                          ...
                          <div
                            onClick={() =>
                              setRegProfil(ProfileType.Commissionnaire)
                            }
                            className={`p-3 rounded-2xl border-2 transition-all flex items-center gap-3 cursor-pointer ${
                          ...
                          <div
                            onClick={() =>
                              setRegProfil(ProfileType.Manutentionnaire)
                            }
                            className={`p-3 rounded-2xl border-2 transition-all flex items-center gap-3 cursor-pointer ${
                      ...
                      {regStep === 2 && (
                        <form
                          ...
                          className="space-y-4 text-xs font-semibold max-h-[50vh] overflow-y-auto pr-2"
                        >
                          ...
                          {regProfil === ProfileType.DonneurOrdre && (
                            <div className="space-y-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[10px] font-black text-slate-450 uppercase tracking-widest block border-b pb-1">
                                {lang === "ar"
                                  ? "معلومات استغلال الشحن والتعاقد"
                                  : "Informations d'exploitation d'affrètement"}
                              </span>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  ...
                                  <select
                                    value={regDoTypeEntite}
                                    onChange={(e) =>
                                      setRegDoTypeEntite(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="Entreprise">
                                      {lang === "ar"
                                        ? "شركة معتمدة (ذ.م.م، ش.ذ.م.م، ش.م)"
                                        : "Entreprise certifiée (EURL, SARL, SPA)"}
                                    </option>
                                    <option value="Artisan">
                                      {lang === "ar"
                                        ? "حرفي / موزع مستقل"
                                        : "Artisan / Distributeur"}
                                    </option>
                                ...
                                {regDoTypeEntite !== "Particulier" && (
                                  <div>
                                    ...
                                    <input
                                      ...
                                      placeholder={
                                        lang === "ar"
                                          ? "مثال: الشركة الجزائرية للباتيمكس"
                                          : "ex: SPA Batimex Algérie"
                                      }
                                      value={regDoRaisonSociale}
                                      onChange={(e) =>
                                        setRegDoRaisonSociale(e.target.value)
                                      }
                                      className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700"
                                    />
                                  </div>
                              ...
                              {regDoTypeEntite !== "Particulier" && (
                                <div className="grid grid-cols-2 gap-2.5">
                                  <div>
                                    ...
                                    <input
                                      type="text"
                                      placeholder="ex: 16/00-0943521B21"
                                      value={regDoRC}
                                      onChange={(e) =>
                                        setRegDoRC(e.target.value)
                                      }
                                      className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 font-mono"
                                    />
                                  </div>
                                  <div>
                                    ...
                                    <input
                                      type="text"
                                      placeholder="ex: 000123567123456"
                                      value={regDoNIF}
                                      onChange={(e) =>
                                        setRegDoNIF(e.target.value)
                                      }
                                      className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 font-mono"
                                    />
                                  </div>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  ...
                                  <select
                                    value={regDoSecteur}
                                    onChange={(e) =>
                                      setRegDoSecteur(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="BTP">
                                      {lang === "ar"
                                        ? "البناء، الأشغال العمومية ومواد البناء"
                                        : "BTP, Cimenterie & Matériaux de Construction"}
                                    </option>
                                    <option value="Agroalimentaire">
                                      {lang === "ar"
                                        ? "الصناعات الغذائية والمشروبات"
                                        : "Boissons & Agroalimentaire"}
                                    </option>
                                    <option value="Chimie">
                                      {lang === "ar"
                                        ? "المواد الكيميائية والبلاستيكية والمحروقات"
                                        : "Chimie, Plasturgie & Carburants"}
                                    </option>
                                    <option value="Électroménager">
                                      {lang === "ar"
                                        ? "الإلكترونيات والأجهزة الكهرومنزلية"
                                        : "Électronique & Électroménager"}
                                    </option>
                                    <option value="Import/Export">
                                      {lang === "ar"
                                        ? "الاستيراد والتصدير والحاويات"
                                        : "Import / Export & Conteneurs"}
                                    </option>
                                    <option value="Autre">
                                      {lang === "ar"
                                        ? "قطاعات صناعية أخرى"
                                ...
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "حجم الشحن الشهري التقديري"
                                      : "Volume d'expédition estimé"}
                                  </label>
                                  <select
                                    value={regDoVolume}
                                    onChange={(e) =>
                                      setRegDoVolume(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="Ponctuel">
                                      {lang === "ar"
                                        ? "أقل من 5 شاحنات في الشهر"
                                        : "Moins de 5 camions par mois"}
                                    </option>
                                    <option value="5-20 camions">
                                      {lang === "ar"
                                        ? "بين 5 و 20 شاحنة في الشهر"
                                        : "Entre 5 et 20 camions par mois"}
                                    </option>
                                    <option value="20-50 camions">
                                      {lang === "ar"
                                        ? "بين 20 و 50 شاحنة في الشهر"
                                        : "Entre 20 et 50 camions par mois"}
                                    </option>
                                    <option value="50+ camions">
                                      {lang === "ar"
                                        ? "أكثر من 50 شاحنة في الشهر"
                                        : "Plus de 50 camions (Flux cadencés)"}
                                    </option>
                                  </select>
                                </div>
                              </div>
                          ...
                          {regProfil === ProfileType.Transporteur && (
                            <div className="space-y-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[10px] font-black text-slate-450 uppercase tracking-widest block border-b pb-1">
                                {lang === "ar"
                                  ? "المعلومات اللوجستية لأسطول النقل"
                                  : "Spécificités logistiques de la flotte d'Algérie"}
                              </span>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "الوضع القانوني للنشاط *"
                                      : "Statut juridique d'activité *"}
                                  </label>
                                  <select
                                    value={regTransTypeEntite}
                                    onChange={(e) =>
                                      setRegTransTypeEntite(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="Artisan transporteur">
                                      {lang === "ar"
                                        ? "ناقل حرفي (مستقل / سائق مستغل)"
                                        : "Artisan Transporteur (Indépendant / Chauffeur-propriétaire)"}
                                    </option>
                                    <option value="Entreprise de transport">
                                      {lang === "ar"
                                        ? "شركة نقل بري ولوجستيات (ش.ذ.م.م أو ش.م)"
                                        : "SARL ou SPA de Transport Logistique"}
                                    </option>
                                  </select>
                                </div>
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "اسم الشركة أو اسم الاستغلال التجاري *"
                                      : "Nom d'exploitation ou Raison Sociale *"}
                                  </label>
                                  <input
                                    type="text"
                                    required
                                    placeholder={
                                      lang === "ar"
                                        ? "مثال: ترانس الصحراء للوجستيات"
                                        : "ex: SARL Trans-Sahara"
                                    }
                                    value={regTransRaisonSociale}
                                    onChange={(e) =>
                                      setRegTransRaisonSociale(e.target.value)
                                    }
                                    className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700"
                                  />
                                </div>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "رقم السجل التجاري (RC) إلزامي *"
                                      : "N° Registre de Commerce (RC) obligatoire *"}
                                  </label>
                                  <input
                                    type="text"
                                    required
                                    placeholder={
                                      lang === "ar"
                                        ? "أدخل رقم السجل التجاري للنقل"
                                        : "N° RC de transporteur"
                                    }
                                    value={regTransRC}
                                    onChange={(e) =>
                                      setRegTransRC(e.target.value)
                                    }
                                    className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 font-mono"
                                  />
                                </div>
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "رقم ترخيص النقل البري للبضائع *"
                                      : "Titre d'autorisation de transport routier *"}
                                  </label>
                                  <input
                                    type="text"
                                    required
                                    placeholder={
                                      lang === "ar"
                                        ? "رقم الاعتماد أو رخصة ممارسة النشاط"
                                        : "N° Certificat Ministériel de transport"
                                    }
                                    value={regTransAutorisation}
                                    onChange={(e) =>
                                      setRegTransAutorisation(e.target.value)
                                    }
                                    className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 font-mono"
                                  />
                                </div>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "الولاية الرئيسية للنشاط (المنطقة)"
                                      : "Wilaya d'activité principale (Région)"}
                                  </label>
                                  <select
                                    value={regTransWilayaActivite}
                                    onChange={(e) =>
                                      setRegTransWilayaActivite(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    {WILAYAS.map((w) => (
                                ...
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "حجم وحجم الأسطول (عدد الشاحنات)"
                                      : "Taille de votre flotte (camions)"}
                                  </label>
                                  <select
                                    value={regTransNbCamions}
                                    onChange={(e) =>
                                      setRegTransNbCamions(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-[#E5E7EB] rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="1">
                                      {lang === "ar"
                                        ? "شاحنة واحدة (سائق مالك فريد)"
                                        : "1 camion (Mon Chauffeur unique)"}
                                    </option>
                                    <option value="2-5">
                                      {lang === "ar"
                                        ? "من 2 إلى 5 شاحنات عاملة"
                                        : "2 à 5 camions opérationnels"}
                                    </option>
                                    <option value="5-15">
                                      {lang === "ar"
                                        ? "من 5 إلى 15 شاحنة ثقيلة"
                                        : "5 à 15 camions lourds"}
                                    </option>
                                    <option value="15+">
                                      {lang === "ar"
                                        ? "أكثر من 15 مقطورة سيمي-رومورك"
                                        : "Plus de 15 semi-remorques"}
                                    </option>
                                  </select>
                                </div>
                              </div>
                          ...
                          {regProfil === ProfileType.Commercial && (
                            <div className="space-y-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[10px] font-black text-slate-450 uppercase tracking-widest block border-b pb-1">
                                {lang === "ar"
                                  ? "تأهيل وكالة وتنمية المبيعات"
                                  : "Qualification Réseau Apporteur d'affaires"}
                              </span>
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "أعلى شهادة علمية حاصل عليها *"
                                      : "Plus haut diplôme *"}
                                  </label>
                                  <select
                                    value={regCommDiplome}
                                    onChange={(e) =>
                                      setRegCommDiplome(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="Master/Ingénieur">
                                      {lang === "ar"
                                        ? "ماستر جامعي أو مهندس دولة في اللوجستيات"
                                        : "Master universitaire ou Ingénieur logistique"}
                                    </option>
                                    <option value="Licence">
                                      {lang === "ar"
                                        ? "ليسانس في التسيير أو العلوم التجارية"
                                        : "Licence en gestion / sciences commerciales"}
                                    </option>
                                    <option value="TS Logistique">
                                      {lang === "ar"
                                        ? "تقني سامي في التسيير واللوجستيات"
                                        : "Technicien Supérieur (TS) en Logistique"}
                                    </option>
                                    <option value="Bac">
                                      {lang === "ar"
                                        ? "التعليم الثانوي أو البكالوريا"
                                        : "Études secondaires ou Baccalauréat"}
                                    </option>
                                  </select>
                                </div>
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "ولاية التدخل التجاري الرئيسية *"
                                      : "Wilaya d'intervention principale *"}
                                  </label>
                                  <select
                                    value={regCommWilayaInterv}
                                    onChange={(e) =>
                                      setRegCommWilayaInterv(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    {WILAYAS.map((w) => (
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "هل لديك خبرة سابقة في الشحن اللوجستي؟"
                                      : "Expérience dans le transport routier ?"}
                                  </label>
                                  <select
                                    value={regCommExperience}
                                    onChange={(e) =>
                                      setRegCommExperience(e.target.value)
                                    }
                                    className="w-full px-2 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700 cursor-pointer"
                                  >
                                    <option value="Oui">
                                      {lang === "ar"
                                        ? "نعم (أكثر من سنتين في مجال النقل)"
                                        : "Oui (Plus de 2 ans dans le domaine logistique)"}
                                    </option>
                                    <option value="Débutant">
                                      {lang === "ar"
                                        ? "لا (رغبة في التدريب وبدء مسار تجاري)"
                                        : "Non (Débutant ou reconversion commerciale)"}
                                    </option>
                                  </select>
                                </div>
                                <div>
                                  <label className="block text-[10px] text-slate-500 mb-0.5">
                                    {lang === "ar"
                                      ? "كيف تعرفت على NETLOG-BVF؟ *"
                                      : "Comment avez-vous connu NETLOG-BVF ? *"}
                                  </label>
                                  <input
                                    type="text"
                                    required
                                    placeholder={
                                      lang === "ar"
                                        ? "توصية، مواقع التواصل الاجتماعي، إلخ"
                                        : "Bouche à oreille, réseaux sociaux..."
                                    }
                                    value={regCommSource}
                                    onChange={(e) =>
                                      setRegCommSource(e.target.value)
                                    }
                                    className="w-full px-2.5 py-1.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-700"
                                  />
                                </div>
                          ...
                          <div className="flex items-start gap-2.5 pt-1">
                            <input
                              ...
                              onChange={(e) =>
                                setRegAcceptCGU(e.target.checked)
                              }
                              className="mt-0.5 accent-[#1D9E75] cursor-pointer"
                            />
                            <label
                              htmlFor="reg-accept-cgu-form"
                              className="text-[10.5px] leading-relaxed text-slate-500 font-semibold"
                            >
                              {lang === "ar" ? (
                                <>
                                  أنا أقبل بدون تحفظ{" "}
                                  <button
                                    ...
                                    className="text-[#1D9E75] font-bold hover:underline bg-transparent border-0 cursor-pointer inline p-0"
                                  >
                                    شروط الاستخدام العامة
                                  </button>{" "}
                                  لمنصة NETLOG وألتزم بتقديم وثائق إدارية صحيحة
                                  وهويات حقيقية.
                                </>
                              ) : (
                                <>
                                  ...
                                  de la plateforme NETLOG et m'engage à fournir
                                  des justificatifs administratifs authentiques.
                                </>
                              )}
                            </label>
                          </div>
                          ...
                          >
                            {lang === "ar"
                              ? "إنهاء عملية التسجيل والتحقق من حسابي ➔"
                              : "Finaliser mon inscription et valider KYC ➔"}
                          </button>
                        </form>
                      )}
                    </div>
                ...
                <div className="pt-6 border-t border-slate-100 text-center space-y-2">
                  <p className="text-[10px] text-slate-400 font-semibold">
                    NETLOG sécurise l'intégralité des flux logistiques en
                    Algérie sous le contrôle de BVF Algérie.
                  </p>
                  ...
                  <button
                    onClick={() => {
                      setGuestMode(true);
                      triggerSystemLog(
                        "Bienvenue en mode invité (Accès public anonyme). Certaines actions d'affrètement exclusives nécessiteront un profil valide.",
                        "info",
                      );
                    }}
          ...
          <>
              ...
              <div className="space-y-6">
                ...
                {!currentUser && (
                  <div
                    ...
                    className="rounded-[16px] p-6 text-white text-center relative overflow-hidden shadow-sm shadow-[#1d9e7520] mb-6"
                  >
                    <div className="relative z-10 space-y-2 max-w-2xl mx-auto">
                      ...
                      <p className="text-xs md:text-sm text-emerald-100 font-semibold opacity-90">
                        600 000+ camions · 58 wilayas · Gratuit pour les
                        donneurs d'ordre
                      </p>
                      <div className="pt-2">
                        <button
                ...
                <div className="sticky top-[56px] z-30 bg-white border border-[#E5E7EB] rounded-[12px] shadow-[0_4px_10px_rgba(0,0,0,0.04)] p-4 space-y-3 mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                    <div className="relative">
                      <span className="absolute left-3.5 top-3 text-xs text-slate-400">
                        📍
                      </span>
                      <select
                        value={filterDepart}
                        onChange={(e) => {
                          ...
                          triggerSystemLog(
                            `Filtre départ réglé sur: ${e.target.value}`,
                            "info",
                          );
                        }}
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          triggerSystemLog(
                            `Filtre arrivée réglé sur: ${e.target.value}`,
                            "info",
                          );
                        }}
                        className="w-full pl-8 pr-3 py-2.5 bg-white border border-[#E5E7EB] rounded-[8px] text-xs font-bold text-slate-700 focus:outline-none focus:border-[#1D9E75] cursor-pointer"
                  ...
                  <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
                    <div className="flex-1 relative">
                      ...
                      >
                        ...
                        <option value={MoyenType.Tautliner}>
                          Tautliner (bâché)
                        </option>
                        ...
                        <option value={MoyenType.BenneBasculante}>
                          Benne basculante
                        </option>
                        <option value={MoyenType.CamionFrigorifique}>
                          Frigorifique
                        </option>
                        <option value={MoyenType.PorteEngin}>
                          Porte-engin
                        </option>
                        <option value={MoyenType.VUL}>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <button
                        onClick={() =>
                          triggerSystemLog(
                            "Recherche actualisée sur la Bourse !",
                            "success",
                          )
                        }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  <div className="card p-4 text-center flex items-center justify-center gap-3 border-l-4 border-l-[#1D9E75]">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-[10px] text-[#6B7280] font-bold uppercase tracking-wide">
                        camions inscrits
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-bold text-[#085041]">
                        Offres de fret disponibles
                      </span>
                      <span className="bg-[#E1F5EE] text-[#085041] rounded-[20px] px-2.5 py-0.5 text-xs font-bold">
                        {filteredBourseOffres.length} offres
                      </span>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-500 dark:text-slate-400 shadow-sm animate-fade-in flex flex-col items-center justify-center space-y-4">
                      ...
                      <div className="space-y-1">
                        <p className="font-black text-[#1A1A2E] dark:text-white text-xs uppercase tracking-wide">
                          {lang === "ar"
                            ? "لا توجد عروض نقل تطابق معاييرك الحالية"
                            : "Aucune offre de transport disponible"}
                        </p>
                        <p className="text-[11px] text-slate-400 dark:text-slate-400 font-semibold leading-normal animate-pulse">
                          {lang === "ar"
                            ? "حاول تغيير ولاية المغادرة أو الوصول لتوسيع البحث."
                            : "Essayez de relâcher les Wilayas de départ ou d'arrivée de vos filtres."}
                        </p>
                      </div>
                    </div>
                  ) : (
                      ...
                      {[...filteredBourseOffres]
                        .sort((a, b) => {
                          if (sortBy === "recent") {
                            return (
                              new Date(
                                b.dateCreation || b.dateChargement,
                              ).getTime() -
                              new Date(
                                a.dateCreation || a.dateChargement,
                              ).getTime()
                            );
                        ...
                        .map((offre) => {
                          ...
                          if (offre.moyenExige === MoyenType.CamionFrigorifique)
                            badgeColor = "badge-bleu";
                          else if (
                            offre.moyenExige === MoyenType.BenneBasculante
                          )
                            badgeColor = "badge-orange";
                          else if (offre.moyenExige === MoyenType.Plateau)
                            badgeColor = "badge-rouge";
                              ...
                              <div>
                                ...
                                <div className="flex justify-between items-center mb-4">
                                  ...
                                  <span className="text-[10px] text-slate-400 font-mono font-bold uppercase group-hover:text-[#1D9E75] transition-colors">
                                    Réf:{" "}
                                    {offre.id.toUpperCase().substring(0, 6)} 🔍
                                  </span>
                                </div>
                                ...
                                <div className="py-2.5 px-3 bg-slate-50 group-hover:bg-emerald-50/20 rounded-xl mb-4 border border-dashed border-slate-100 flex items-center justify-between transition-colors">
                                  ...
                                  <span className="text-[#1D9E75] font-black group-hover:translate-x-1 transition-transform">
                                    ➔
                                  </span>
                                  <div className="font-black text-sm text-[#085041] flex items-center gap-1.5">
                                    <span className="text-xs">📍</span>{" "}
                                  ...
                                  {isOwner ? (
                                    <button
                                      onClick={() => {
                                        setCurrentTab("donneur");
                                        triggerSystemLog(
                                          "Rendu sur votre espace personnel Donneur pour administrer le fret",
                                          "info",
                                        );
                                      }}
                                    ...
                                    <button
                                      onClick={() => {
                                        triggerSystemLog(
                                          "Pour proposer une cotation, veuillez vous connecter comme Transporteur (en haut ou via le menu !)",
                                          "info",
                                        );
                                      }}
                  ...
                  <div className="mt-10 pt-8 border-t border-[#E5E7EB]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="card p-5 bg-white text-center space-y-2 border border-gray-100">
                        ...
                        <h4 className="font-bold text-xs text-[#085041] uppercase tracking-wider">
                          Trouvez rapidement
                        </h4>
                        <p className="text-xs text-[#6B7280] leading-relaxed font-semibold">
                          Recherchez instantanément parmi des centaines de
                          transporteurs qualifiés et d'offres de transport
                          partout en Algérie.
                        </p>
                      </div>
                      <div className="card p-5 bg-white text-center space-y-2 border border-gray-100">
                        ...
                        <p className="text-xs text-[#6B7280] leading-relaxed font-semibold">
                          Évitez les intermédiaires gourmands en marges
                          d'affrètement. Des cotations réelles directes
                          conformes au marché national.
                        </p>
                      </div>
                      <div className="card p-5 bg-white text-center space-y-2 border border-gray-100">
                        <div className="text-3xl">📊</div>
                        ...
                        <p className="text-xs text-[#6B7280] leading-relaxed font-semibold">
                          Sécurisez vos chargements grâce au logigramme
                          dynamique et l'authentification des décharges par
                          codes uniques.
                        </p>
                      </div>
                    </div>
            ...
            {currentTab === "admin" &&
              currentUser?.profil === ProfileType.Admin && (
                <AdminDashboard
                  currentUser={currentUser}
                  setCurrentUser={setCurrentUser}
                  lang={lang}
              ...
              currentUser?.profil === ProfileType.Admin && (
                <div className="space-y-6">
                  ...
                  <div
                    ...
                    className="bg-slate-900 text-white p-6 rounded-3xl shadow-md border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4"
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-400 font-mono">
                        Session ID: {currentUser.id} • Droits: Contrôle Total,
                        KYC Approbation, Modérateur Système
                      </p>
                    </div>
                      ...
                      <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                        <div className="flex items-center justify-between border-b pb-3 border-gray-50">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400">
                                Examinez et validez les documents légaux
                                d'inscription des nouveaux transporteurs et
                                donneurs d'ordres.
                              </p>
                            </div>
                          </div>
                          ...
                          <span className="bg-amber-100 text-amber-800 text-[10px] font-black px-2.5 py-0.5 rounded-full animate-pulse">
                            {
                              users.filter((u) => u.status === "en_attente")
                                .length
                            }{" "}
                            En Attente
                          </span>
                        ...
                        {users.filter((u) => u.status === "en_attente")
                          ...
                          <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-gray-150 text-xs font-semibold text-slate-400">
                            ...
                            administrative. Tous les membres sont d'équerre !
                          </div>
                        ) : (
                          <div className="space-y-4">
                            {users
                              ...
                              .map((u) => (
                                <div
                                  key={u.id}
                                  className="p-4 rounded-2xl bg-amber-50/10 border border-amber-200/50 space-y-3.5"
                                >
                        ...
                        <div className="space-y-2.5">
                          {users.map((u) => {
                            ...
                            return (
                              <div
                                key={u.id}
                                className="p-3 bg-slate-50/60 rounded-2xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs"
                              >
                                <div className="space-y-0.5">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className="font-extrabold text-[#1a1a2e] text-[12px]">
                                      {u.raisonSociale}
                                    </span>
                                    <span
                                      className={`text-[8.5px] font-black uppercase px-1.5 py-0.5 rounded tracking-wider ${
                                        u.profil === ProfileType.DonneurOrdre
                                          ? "bg-blue-100 text-[#0C447C]"
                                          : u.profil ===
                                              ProfileType.Transporteur
                                            ? "bg-emerald-100 text-[#085041]"
                                            : "bg-purple-100 text-[#3B1E7B]"
                                      }`}
                                    >
                                      {u.profil}
                                    </span>
                                  ...
                                  <span className="text-[10px] text-slate-400 block font-sans">
                                    Contact: {u.prenom} {u.nom} • {u.email} •
                                    Tel: {u.tel}
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 self-end sm:self-center">
                                  <span
                                    className={`border px-2 py-0.5 rounded-full text-[9px] font-black ${tagBg}`}
                                  >
                                    {tagLabel}
                                  </span>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <div>
                              ...
                              <span className="text-[9px] text-slate-400 block font-normal">
                                Approuve instantanément tous les pro-formas
                              </span>
                            </div>
                          </button>
                        </div>
                      ...
                      <div className="bg-white p-5 rounded-3xl border border-[#E1F5EE] shadow-sm space-y-3 bg-[#E1F5EE]/10">
                        ...
                        <div className="text-[10px] text-[#085041] leading-relaxed font-semibold space-y-1.5 font-sans">
                          <p>
                            Conformément à la Loi d'Affrètement Routier de Fret
                            en Algérie :
                          </p>
                          <ul className="list-disc pl-3.5 space-y-1">
                            <li>
                              Toute validation de KYC nécessite un Registre du
                              Commerce d'Activité de Transport ou Entreprise
                              valide.
                            </li>
                            <li>
                              Le code unique à 4 chiffres sécurise le transfert
                              de responsabilité à destination.
                            </li>
                            <li>
                              Les réserves écrites doivent être visées
                              contradictoirement lors de la livraison du fret.
                            </li>
                          </ul>
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-slate-500 font-sans leading-normal">
                        Visualisez les corridors logistiques nationaux et les
                        volumes de camions en temps réel par wilayas clés
                        d'exploitation.
                      </p>
                    </div>
                    ...
                    <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-3xl p-4 border border-slate-200 text-slate-800 dark:text-slate-200 min-h-[460px] flex flex-col justify-between relative overflow-hidden shadow-inner">
                      ...
                      <div className="flex gap-4 items-center border-t border-slate-100 dark:border-slate-800 pt-3 mt-3 relative z-10">
                        ...
                        <p className="text-[10px] text-slate-500 font-sans leading-normal">
                          Couplé au GPS de la flotte des transporteurs NETLOG.
                          Les calculs d'itinéraires intègrent les corridors
                          logistiques nationaux et l'état des routes nationales
                          (RN1, RN3, RN5).
                        </p>
                      </div>
                    </div>
            ...
            {currentTab === "publier" &&
              (() => {
                ...
                const handleQuickPublishOfferSubmit = async (
                  e: React.FormEvent,
                ) => {
                  e.preventDefault();
                  if (!isDO) {
                    triggerSystemLog(
                      "Interdit : Vous devez être connecté en tant que Donneur d'Ordre pour poster une offre.",
                      "danger",
                    );
                    return;
                  }

                  const wilayaDepartObj = WILAYAS.find(
                    (w) => w.fr === pubDepart,
                  );
                  const wilayaArriveeObj = WILAYAS.find(
                    (w) => w.fr === pubArrivee,
                  );
                  if (!wilayaDepartObj || !wilayaArriveeObj) {
                    triggerSystemLog(
                      "Wilaya de départ ou d'arrivée invalide.",
                      "danger",
                    );
                    return;
                  }
                  ...
                  try {
                    const inserted = await createFreightOffer({
                      ...
                      description: pubCommentaire || undefined,
                      poidsKg: pubPoids ? Number(pubPoids) * 1000 : undefined,
                      typeMarchandise: pubMarchandise || undefined,
                      prixPropose: pubPrix ? Number(pubPrix) : 0,
                      paymentMethod: "cash",
                      dateEnlevementSouhaitee: new Date(
                        ...
                        .toISOString()
                        .split("T")[0],
                      typeMoyenExige: pubMoyen || undefined,
                      nombreVoyages: 1,
                    });
                    insertedId = inserted.id;
                  } catch (err: any) {
                  ...
                  const newOffer: OffreFret = {
                    ...
                    donneurRaisonSociale:
                      currentUser.raisonSociale ||
                      `${currentUser.prenom} ${currentUser.nom}`,
                    depart: pubDepart,
                    arrivee: pubArrivee,
                    departDetails: `Entrepôt principal de ${pubDepart}`,
                    arriveeDetails: `Dépôt client d'arrivée à ${pubArrivee}`,
                    dateChargement: new Date(Date.now() + 86400000 * 3)
                      .toISOString()
                      .split("T")[0],
                    dateLivraison: new Date(Date.now() + 86400000 * 5)
                      .toISOString()
                      .split("T")[0],
                    poids: pubPoids,
                      ...
                      <div className="bg-white p-8 rounded-3xl border border-rose-100 shadow-md text-center max-w-2xl mx-auto space-y-5">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <p className="text-xs text-slate-500 leading-relaxed max-w-md mx-auto">
                            La publication directe d'offres de fret ou de
                            demandes de transport sur la bourse NETLOG nécessite
                            un compte professionnel de type{" "}
                            <strong className="text-slate-900">
                              Donneur d'Ordre d'entreprise
                            </strong>{" "}
                            (Shipper).
                          </p>
                        </div>
                      ...
                      <div className="max-w-3xl mx-auto bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6">
                        <div className="border-b border-gray-100 pb-4">
                          ...
                          <p className="text-[11px] text-slate-500 font-sans mt-0.5 leading-normal">
                            Remplissez les détails logistiques de votre
                            chargement routier. Une fois validée, l'offre
                            apparaîtra aux transporteurs certifiés KYC
                            d'Algérie.
                          </p>
                        </div>

                        <form
                          onSubmit={handleQuickPublishOfferSubmit}
                          className="space-y-5"
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1.5 text-xs text-slate-600 font-bold">
                              <label>
                                🏁 Ville de Déchargement (Destination) *
                              </label>
                              <select
                                value={pubArrivee}
                          ...
                          {/* Form validators */}
                          {(() => {
                            const isMarchandiseValid =
                              pubMarchandise.trim().length >= 3;
                            const isPoidsValid =
                              pubPoids > 0 && pubPoids <= 100;
                            const isPrixValid = pubPrix >= 5000;
                            const isFormValid =
                              isMarchandiseValid && isPoidsValid && isPrixValid;
                              ...
                              <>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                  ...
                                  <div className="space-y-1.5 text-xs text-slate-600 font-bold">
                                    ...
                                    <select
                                      value={pubMoyen}
                                      onChange={(e) =>
                                        setPubMoyen(e.target.value as MoyenType)
                                      }
                                      className="w-full p-2.5 bg-slate-50 border border-slate-200 text-slate-900 rounded-xl font-bold font-sans text-xs focus:ring-[#1D9E75] focus:border-[#1D9E75] cursor-pointer"
                                    >
                                      {Object.values(MoyenType).map((t) => (
                                        <option key={t} value={t}>
                                          {translateMoyenType(t, lang)}
                                        </option>
                                      ))}
                                    </select>
                                  </div>
                                  ...
                                  <div className="space-y-1.5 text-xs text-slate-600 font-bold">
                                    ...
                                    <input
                                      ...
                                      value={pubPoids}
                                      onChange={(e) =>
                                        setPubPoids(Number(e.target.value))
                                      }
                                  ...
                                  <div className="space-y-1.5 text-xs text-slate-600 font-bold">
                                    ...
                                    <input
                                      ...
                                      value={pubPrix}
                                      onChange={(e) =>
                                        setPubPrix(Number(e.target.value))
                                      }
                                ...
                                <div className="space-y-1.5 text-xs text-slate-600 font-bold">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setPubMarchandise(e.target.value)
                                    }
                                    className={`w-full p-2.5 bg-slate-50 border rounded-xl font-bold text-xs outline-none transition-all ${
                                      pubMarchandise.length === 0
                                        ? "border-slate-200 focus:border-slate-400"
                                        : isMarchandiseValid
                                          ? "border-emerald-500 focus:border-emerald-600"
                                          : "border-rose-500 focus:border-rose-600"
                                  ...
                                  {pubMarchandise.length > 0 &&
                                    !isMarchandiseValid && (
                                      <p className="text-[10px] text-rose-500 font-bold">
                                        ⚠️ Saisissez au moins 3 caractères
                                      </p>
                                    )}
                                </div>
                                ...
                                <div className="space-y-1.5 text-xs text-slate-600 font-bold font-sans">
                                  <label>
                                    📝 Instructions particulières, exigences et
                                    manutention
                                  </label>
                                  <textarea
                                    rows={3}
                                    ...
                                    onChange={(e) =>
                                      setPubCommentaire(e.target.value)
                                    }
                                ...
                                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100/60 text-[10px] text-slate-500 leading-relaxed font-semibold">
                                  <p className="font-bold text-slate-700">
                                    📜 Charte d'Affrètement NETLOG Algérie :
                                  </p>
                                  En soumettant cette demande de fret, vous vous
                                  engagez à détenir la propriété des biens à
                                  transporter ou un mandat licite, et à honorer
                                  la facture émise consécutivement au code de
                                  validation transmis par votre destinataire
                                ...
                                <button
                                  ...
                                  className={`w-full py-3.5 text-white text-xs font-black uppercase tracking-wider rounded-xl cursor-pointer shadow-sm transition ${
                                    isFormValid
                                      ? "bg-[#D85A30] hover:bg-orange-700 active:scale-[0.98]"
                                      : "bg-slate-300 cursor-not-allowed opacity-60"
                                  }`}
                                >
                                  🚀{" "}
                                  {isFormValid
                                    ? "Publier ma commande de fret"
            ...
            {currentTab === "menu" &&
              (() => {
                const baseMonthlyRate =
                  subSelectedPlan === "free"
                    ? 0
                    : subSelectedPlan === "premium"
                      ? 4900
                      : 9900;
                ...
                const handleActivateSubscriptionSelection = () => {
                  if (subSelectedPlan === "free") {
                    triggerSystemLog(
                      "Plan standard d'évaluation activé par défaut (Sans frais).",
                      "info",
                    );
                    return;
                  }
                  ...
                  triggerSystemLog(
                    `Fiche de paiement Pro-forma générée ! CCP NETLOG Algérie Clé 52. Abonnement ${subSelectedPlan.toUpperCase()} de ${subSelectedDuration} mois pré-activé.`,
                    "success",
                  );
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-2 border-b border-slate-100 pb-3 flex-wrap">
                      ...
                      >
                        ...
                        <span className="bg-[#1D9E75] text-white text-[9px] px-1.5 py-0.5 rounded-full font-black animate-pulse">
                          PRO
                        </span>
                      </button>
                      <button
                      ...
                      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-5">
                        <div className="border-b border-slate-50 pb-3">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            Basculez instantanément d'un profil métier à un
                            autre pour examiner l'ensemble des modules
                            interactifs, du chargement routier à la facturation.
                          </p>
                        </div>
                            ...
                            return (
                              <div
                                ...
                                className={`p-4 rounded-2xl border transition duration-150 flex flex-col justify-between space-y-3 ${
                                  isActive
                                    ? "border-[#1D9E75] bg-[#E1F5EE]/10 shadow-sm"
                              ...
                              >
                                <div className="space-y-1">
                                  <span
                                    className={`text-[8.5px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${
                                      profileUser.profil === ProfileType.Admin
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    triggerSystemLog(
                                      `Identité simulée changée pour : ${profileUser.raisonSociale || profileUser.nom}`,
                                      "success",
                                    );
                                  }}
                        ...
                        <div className="lg:col-span-8 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-slate-400 font-sans leading-normal">
                              Les transporteurs d'Algérie doivent souscrire à
                              une formule pour ajouter des véhicules routiers à
                              leur flotte d'affrètement.
                            </p>
                          </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              {[
                                ...
                                >
                                  ...
                                  <span className="text-[9.5px] font-bold text-slate-400 uppercase leading-none">
                                    {formula.desc}
                                  </span>
                                </div>
                              ))}
                          ...
                          <div className="space-y-2">
                            <span className="text-[10px] uppercase font-black tracking-widest text-[#085041] block">
                              Étape 2 : Durée d'engagement (Économisez jusqu'à
                              20%)
                            </span>
                            <div className="grid grid-cols-4 gap-2 text-center text-xs">
                              {[
                                ...
                                <div
                                  key={d.val}
                                  onClick={() =>
                                    setSubSelectedDuration(d.val as any)
                                  }
                                  className={`p-3 rounded-2xl border cursor-pointer transition ${
                                    subSelectedDuration === d.val
                                      ? "border-[#1D9E75] bg-[#E1F5EE]/40 text-slate-900 font-black shadow-xs"
                                      : "border-slate-100 text-slate-500 hover:text-slate-800 hover:bg-slate-50 bg-white"
                                  }`}
                                >
                                  <span className="block text-[11px] font-extrabold">
                                    {d.label}
                                  </span>
                                  <span className="block text-[9.5px] font-bold text-[#1D9E75] uppercase mt-0.5">
                                    {d.disc}
                                  </span>
                                </div>
                              ))}
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="space-y-1.5 text-xs text-slate-300 font-bold">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="text-white font-mono">
                                  {baseMonthlyRate.toLocaleString()} DA/mois
                                </span>
                              </div>
                              <div className="flex justify-between border-t border-slate-800/60 pt-2 text-slate-400">
                                <span>Montant brut total :</span>
                                <span className="font-mono text-white text-xs line-through">
                                  {grossPrice.toLocaleString()} DA
                                </span>
                              </div>
                              {remisePct > 0 && (
                      ...
                      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                        <div className="border-b border-slate-50 pb-2">
                          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest">
                            📜 Mentions Légales, Régulation des Transports et
                            CGU
                          </h3>
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            NETLOG est opéré conformément au cahier des charges
                            national algérien régissant les prestataires
                            d'affrètement de fret routier.
                          </p>
                        </div>

                        <div className="text-xs text-slate-650 leading-relaxed space-y-3 font-sans font-medium">
                          <p>
                            <strong>1. Exploitation de la Bourse :</strong>{" "}
                            L'accès direct à la plateforme d'affrètement NETLOG
                            est strictement interdit à tout transporteur ne
                            détenant pas de Registre du Commerce algérien
                            (activité transports) valide, ainsi que d'une
                            autorisation réglementaire de circulation délivrée
                            par la wilaya d'immatriculation.
                          </p>
                          <p>
                            <strong>
                              2. Responsabilité Civile Professionnelle :
                            </strong>{" "}
                            Chaque chargement validé par le biais de la Lettre
                            de voiture (LDV) réglementaire de transport émise
                            requiert que le transporteur conserve une assurance
                            multirisque de transport de marchandises en cours de
                            validité routière.
                          </p>
                          <p>
                            <strong>3. Solution Administrative BVF :</strong>{" "}
                            Les commissionnaires territoriaux en douane ou BVF
                            opèrent à titre d'agents intermédiaires d'état civil
                            de régulation, contrôlant l'authenticité physique
                            des dépôts de garantie lors de la livraison.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {activeSubTabMenu === "config" && (
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                {lang === "ar"
                                  ? "اختر المظهر المناسب لراحتك البصرية"
                                  : "Gérer les préférences d'affichage visuel de l'application"}
                              </p>
                            </div>
                          ...
                          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                {lang === "ar"
                                  ? "ثبّت التطبيق على شاشتك الرئيسية للوصول المباشر"
                                  : "Accédez à NETLOG directement de votre écran d'accueil sans navigateur"}
                              </p>
                            </div>
                            ...
                            {deferredPrompt ? (
                                ...
                                onClick={async () => {
                                  if (deferredPrompt) {
                                    ...
                                    if (outcome === "accepted") {
                                      showToast(
                                        lang === "ar"
                                          ? "🎉 شكرًا لتثبيتك التطبيق !"
                                          : "🎉 Merci d'avoir installé NETLOG !",
                                        "success",
                                      );
                                    }
                              ...
                              >
                                ...
                                {lang === "ar"
                                  ? "تثبيت الآن"
                                  : "Installer l'application NETLOG"}
                              </button>
                            ) : (
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
                          <div>
                            <h4 className="text-xs font-black uppercase text-amber-600 tracking-widest">
                              🔢{" "}
                              {lang === "ar"
                                ? "العدادات التلقائية المستمرة"
                                : "Compteurs de Séquençage Automatique"}
                            </h4>
                            <p className="text-[10px] text-slate-400 mt-0.5">
                              {lang === "ar"
                                ? "الأرقام النشطة للمستندات والعمليات المسجلة بـ localStorage"
                                : "Valeurs d'indexation persistées localement pour la génération des bordereaux rattachés"}
                            </p>
                          </div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            {[
                              {
                                label: lang === "ar" ? "العروض" : "Offres Fret",
                                value: `NET-OFF-${counters.offres}`,
                                key: "offres",
                              },
                              {
                                label: lang === "ar" ? "المهام" : "Missions",
                                value: `NET-MIS-${counters.missions}`,
                                key: "missions",
                              },
                              {
                                label: lang === "ar" ? "الفواتير" : "Factures",
                                value: `NET-FAC-${counters.factures}`,
                                key: "factures",
                              },
                              {
                                label:
                                  lang === "ar"
                                    ? "رسائل الشحن"
                                    : "Lettres de Voiture",
                                value: `NET-LDV-${counters.ldv}`,
                                key: "ldv",
                              },
                            ].map((c) => (
                              <div
                                key={c.key}
                                className="p-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-between text-center space-y-1.5"
                              >
                                <span className="text-[9.5px] text-slate-400 font-black uppercase tracking-wider">
                                  {c.label}
                                </span>
                                <span className="font-mono text-xs font-black text-[#1D9E75]">
                                  {c.value}
                                </span>
                                <button
                        ...
                        <div
                          style={{ backgroundColor: "#0f172a" }}
                          className="bg-slate-900 text-white rounded-3xl p-6 border border-slate-800 shadow-md flex flex-col sm:flex-row gap-6 justify-between items-center"
                        >
                          <div className="space-y-1">
                            <h4 className="text-sm font-black uppercase tracking-wider text-teal-400 font-mono">
                              NETLOG – Bourse de Fret Algérie v2.1
                            </h4>
                            <p className="text-[10.5px] text-slate-300 leading-normal max-w-xl">
                              Née pour digitaliser les chaînes de valeur de fret
                              lourd méditerranéen vers le Sahara algérien.
                              NETLOG est la première solution intégrée
                              d'affrètement routier avec KYC.
                            </p>
                          </div>
              ...
              <div className="max-w-xl mx-auto bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                <div className="text-center space-y-1">
                  ...
                  <p className="text-[11px] text-slate-400 font-sans">
                    Sélectionnez l'un de nos profils pré-configurés de test pour
                    accéder instantanément à ses données d'exploitation.
                  </p>
                </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (profileUser.profil === ProfileType.DonneurOrdre) {
                          ...
                          profileUser.profil === ProfileType.Transporteur
                        ) {
                          setCurrentTab("transporteur");
                        } else if (
                          profileUser.profil === ProfileType.Commercial
                        ) {
                          setCurrentTab("commercial");
                        ...
                        triggerSystemLog(
                          `Bienvenue ! Connecté en tant que : ${profileUser.raisonSociale || profileUser.prenom}`,
                          "success",
                        );
                      }}
                ...
                {currentUser && (
                  <div className="pt-4 border-t border-slate-100 text-center space-y-2">
                    <p className="text-xs text-slate-500">
                      ...
                      <strong className="text-slate-900">
                        {currentUser.raisonSociale || currentUser.nom}
                      </strong>
                    </p>
                    <button
                      onClick={() => {
      ...
      <footer className="bg-slate-900 text-slate-400 py-6 border-t border-slate-800 text-xs mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-2">
          <p className="font-sans font-bold text-[#1D9E75]">
            {lang === "ar"
              ? "NETLOG – بورصة الشحن بالجزائر"
              : "NETLOG – Bourse de Fret Algérie"}
          </p>
          <p className="text-[10px] text-slate-500 font-sans leading-relaxed">
            {lang === "ar" ? (
              <>
                ربط منظم وقانوني بين المتعاملين في شحن البضائع البري، النقل
                الخاص والاستثنائي، والخدمات اللوجستية في الجزائر.
                <br />© {new Date().getFullYear()} شركة ذ.م.م NETLOG. مطابق
                لقوانين شحن واستئجار الطرق الصادرة عن وزارة النقل بالجزائر.
              </>
            ) : (
              <>
                Mise en relation réglementée d'acteurs de fret routier,
                transports spéciaux, et services logistiques en Algérie.
                <br />© {new Date().getFullYear()} NETLOG Sarl. Conforme aux
                règles d'affrètement du Ministère des Transports d'Algérie.
              </>
            )}
          </p>
        </div>
      ...
      {activeContractDoc &&
        (() => {
          const doId =
            activeContractDoc.offre?.donneurId ||
            activeContractDoc.fac?.donneurId;
          const doUser = users.find((u) => u.id === doId) || users[0];
          const prUser =
            users.find(
              (u) =>
                u.id === activeContractDoc.prop?.transporteurId ||
                u.id === activeContractDoc.fac?.transporteurId,
            ) || users[1];
          const usedMoyen = moyens.find(
            (m) => m.id === activeContractDoc.prop?.moyenId,
          );
      ...
      {trackedOffreId &&
        (() => {
              ...
              <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl p-8 max-w-md w-full border border-slate-100 text-center space-y-4">
                  ...
                  <h3 className="font-extrabold text-slate-900 text-base">
                    Recherche de la livraison en cours...
                  </h3>
                  <p className="text-xs text-slate-500 leading-normal">
                    Chiffrement de la liaison de transit en direct. Récupération
                    des informations de l'expédition #{trackedOffreId}...
                  </p>
                  <button
          ...
          const doUser =
            users.find((u) => u.id === trackedOffre.donneurId) ||
            users.find((u) => u.profil === ProfileType.DonneurOrdre) ||
            users[0];
          const transporteurUser =
            users.find((u) => u.profil === ProfileType.Transporteur) ||
            users[1];
          const caniveauMoyen =
            moyens.find((m) => m.transporteurId === transporteurUser.id) ||
            moyens[0];
            ...
            <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
              <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full border border-slate-100 overflow-hidden flex flex-col my-4 max-h-[92vh]">
                ...
                <div className="p-6 md:p-8 overflow-y-auto flex-1 space-y-6 text-slate-800 text-xs">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                    ...
                    <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-2">
                      ...
                      {/* Step 2: Chargé */}
                      {trackedOffre.status === OffreStatus.Attribue ||
                      trackedOffre.status === OffreStatus.Publie ? (
                        <div className="flex items-center gap-3 md:flex-col md:text-center md:flex-1 relative z-10 opacity-70">
                          ...
                          <div>
                            <p className="font-black text-slate-700 text-[11px]">
                              Étape 2 : Chargement
                            </p>
                      ...
                      <div
                        className={`hidden md:block h-0.5 flex-1 mx-2 ${
                          ...
                          trackedOffre.status === OffreStatus.Decharge ||
                          trackedOffre.status === OffreStatus.Valide
                            ? "bg-emerald-600"
                            : "bg-slate-200"
                        }`}
                      />
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[11px]">
                    <div className="p-3.5 border border-slate-150 rounded-xl bg-slate-50 space-y-1">
                      <strong className="text-slate-400 text-[9px] uppercase tracking-wider block">
                        DÉPART (EXPÉDITEUR)
                      </strong>
                      ...
                      <div className="text-slate-500 text-[10px] leading-tight mt-0.5">
                        {trackedOffre.departDetails || "Port National"}
                      </div>
                      <div className="text-[#1D9E75] font-black text-[9px] mt-1.5 uppercase">
                        DO : {doUser.raisonSociale}
                      </div>
                    </div>
                  ...
                  <div className="p-4 border border-slate-150 rounded-2xl bg-[#E1F5EE]/20 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1 leading-normal">
                      ...
                      <strong className="text-slate-900 text-xs">
                        📦 {trackedOffre.marchandise}
                      </strong>
                      <div className="text-slate-500 font-medium text-[10.5px]">
                        Moyen requis : {trackedOffre.moyenExige} • Poids :{" "}
                        <b>{trackedOffre.poids} tonnes</b>
                      </div>
                    </div>

                    <div className="space-y-1 leading-normal">
                      <span className="text-[9px] text-slate-400 uppercase font-black block">
                        Prestataire et Camion
                      </span>
                      <strong className="text-slate-900 text-xs">
                        🚛 {transporteurUser.raisonSociale}
                      </strong>
                      <div className="text-slate-500 font-medium text-[10.5px]">
                        Plaque d'immatr. :{" "}
                        <b className="font-mono bg-slate-100 px-1 py-0.5 rounded text-slate-800">
                          {caniveauMoyen?.immatriculation || "012356-116-31"}
                        </b>
                      </div>
                    </div>
                  </div>
                    ...
                    {(trackedOffre.status === OffreStatus.Attribue ||
                      ...
                      <div className="space-y-3.5">
                        <div className="bg-emerald-50 text-emerald-950 p-3.5 rounded-xl text-xs font-medium border border-emerald-100 leading-normal">
                          <strong>📌 Instruction de chargement :</strong> Le
                          transporteur est arrivé au quai d'enlèvement. Veuillez
                          inspecter la marchandise à charger puis cliquez sur le
                          bouton de validation ci-dessous pour confirmer la
                          prise en charge et le départ de l'expédition.
                        </div>
                    ...
                    {trackedOffre.status === OffreStatus.Charge && (
                      <div className="space-y-4">
                        <div className="bg-amber-50 text-amber-950 p-3.5 rounded-xl font-medium border border-amber-200/50 leading-normal">
                          <strong>
                            📌 Instruction de livraison de destination :
                          </strong>{" "}
                          Le camion s'est présenté au quai de déchargement.
                          Veuillez procéder à la vérification physique des colis
                          et de l'état de la cargaison. Si vous constatez des
                          dommages ou manquants, renseignez les réserves écrites
                          ci-dessous avant d'acquitter, sinon valisez
                          directement.
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center text-[11px] font-bold">
                            <label className="text-slate-700">
                              Mentionner des réserves (optionnel)
                            </label>
                          ...
                          {!trackerNoReserves && (
                            <textarea
                              ...
                              onChange={(e) =>
                                setTrackerReserves(e.target.value)
                              }
                ...
                <div className="bg-slate-50 p-4 border-t border-slate-100 text-center text-[10px] text-slate-400 font-mono">
                  Portail de validation décentralisé NETLOG DZ • Conforme à la
                  trace électronique routière algérienne.
                </div>
              </div>
            </div>
          );
            ...
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div>
                <h3 className="text-sm font-black uppercase text-[#085041] tracking-wider">
                  {lang === "ar"
                    ? "إنشاء حساب مهني NETLOG"
                    : "Rejoindre le Réseau Pro NETLOG"}
                </h3>
                <p className="text-[10px] text-slate-500 mt-0.5">
                  {regStep === 1
                    ? lang === "ar"
                      ? "خطوة 1: اختر نوع حسابك"
                      : "Étape 1 sur 2 : Choisissez votre profil d'activité"
                    : lang === "ar"
                      ? "خطوة 2: أدخل معلوماتك الشخصية والمهنية"
                      : "Étape 2 sur 2 : Informations d'enregistrement"}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            {regStep === 1 && (
              <div className="space-y-4">
                <p className="text-xs font-bold text-slate-700 text-center mb-2">
                  {lang === "ar"
                    ? "ما هو قطاع نشاطك الرئيسي في المنصة؟"
                    : "Quel est le profil qui décrit le mieux votre activité ?"}
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal font-medium">
                      Entreprises, PME, commerçants ou particuliers ayant du
                      fret à faire transporter sur l'Algérie.
                    </p>
                    {regProfil === ProfileType.DonneurOrdre && (
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal font-medium">
                      Camionneurs indépendants, artisans-transporteurs ou
                      entreprises de logistique avec flotte.
                    </p>
                    {regProfil === ProfileType.Transporteur && (
                      <span className="absolute top-2 right-2 w-3 h-3 bg-[#1D9E75] rounded-full ring-2 ring-white"></span>
                    )}
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal font-medium">
                      Agent commercial indépendant de NETLOG, apporteurs
                      d'affaires et recruteurs connectés.
                    </p>
                    {regProfil === ProfileType.Commercial && (
                      <span className="absolute top-2 right-2 w-3 h-3 bg-[#1D9E75] rounded-full ring-2 ring-white"></span>
                    )}
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal font-medium">
                      Prestataires de manutention : grues, chariots élévateurs,
                      pelles, nacelles.
                    </p>
                    {regProfil === ProfileType.Manutentionnaire && (
                      <span className="absolute top-2 right-2 w-3 h-3 bg-[#1D9E75] rounded-full ring-2 ring-white"></span>
              ...
              >
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100/60">
                    ...
                    <div>
                      ...
                      {regPassword && (
                        <div className="mt-1 flex items-center gap-1">
                          <div
                            className={`h-1.5 w-12 rounded ${getPasswordStrength(regPassword).color}`}
                          ></div>
                          <span
                            className={`text-[9px] font-bold ${getPasswordStrength(regPassword).text}`}
                          >
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <label
                      htmlFor="viewPwdReg"
                      className="text-[10px] text-slate-500 font-medium select-none cursor-pointer"
                    >
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="flex gap-4">
                          {["Entreprise", "Commerçant", "Particulier"].map(
                            (v) => (
                              <label
                                key={v}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="ex: 16/00-0943521B21"
                          value={regManRC}
                          onChange={(e) => setRegManRC(e.target.value)}
                          className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs bg-white font-mono"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] text-slate-500 mb-0.5">
                          Type d'engin principal *
                        </label>
                        <select
                          value={regManTypesEngins[0]}
                          onChange={(e) =>
                            setRegManTypesEngins([e.target.value])
                          }
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-[11px] leading-relaxed flex items-start gap-2 select-none">
                  ...
                  <div className="text-slate-600 font-medium">
                    <label htmlFor="acceptCGUID" className="cursor-pointer">
                      ...
                      de la bourse de fret NETLOG en Algérie. *
                    </label>
                    <button
                      type="button"
                      onClick={() => setShowCGUModal(true)}
        ...
        <div className="fixed inset-0 z-50 bg-[#0F172A]/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 border border-slate-100 shadow-2xl relative">
            <div className="text-center relative">
              ...
              <h3 className="text-xs font-black uppercase tracking-wider text-[#085041]">
                {lang === "ar"
                  ? "تسجيل الدخول إلى NETLOG"
                  : "Connexion à NETLOG"}
              </h3>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 text-center text-[11px]">
              <button
                onClick={() =>
                  triggerSystemLog(
                    "Demande de réinitialisation transmise à l'administrateur. Un email de récupération vous sera envoyé sous peu.",
                    "info",
                  )
                }
        ...
        <div className="fixed inset-0 z-50 bg-[#0F172A]/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 border border-slate-100 shadow-2xl relative">
            ...
            <div className="text-[11px] leading-relaxed text-slate-600 font-medium max-h-[50vh] overflow-y-auto space-y-2.5 p-1 pr-2">
              <p>
                <strong>1. Objet :</strong> NETLOG est une plateforme numérique
                de mise en relation de fret routier, d'affrètement national de
                marchandises et de services logistiques réglementés en Algérie.
              </p>
              <p>
                <strong>2. Certifications requises :</strong> Les transporteurs
                s'engagent à fournir des registres de commerce (RC) valides
                ainsi que des autorisations ministérielles réglementaires de
                transport routier.
              </p>
              <p>
                <strong>3. Sécurité des cargaisons :</strong> Le suivi du
                déchargement est validé de manière asymétrique par un code
                unique de confirmation partagé exclusivement entre le donneur
                d'ordre et le transporteur assigné.
              </p>
              <p>
                <strong>4. Tarifs et Commissions :</strong> Les propositions
                tarifaires sont fermes après acceptation d'une soumission. Les
                commerciaux agréés perçoivent une commission légale de 200 DA
                (portefeuille transporteur) ou 500 DA (portefeuille donneur
                d'ordre) par transaction validée.
              </p>
              <p>
                <strong>5. Validation Administrative :</strong> Tout compte
                nouvellement inscrit fait l'objet d'une analyse administrative
                sous 24h par l'équipe support NETLOG.
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-4 border border-slate-100 max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="flex justify-between items-center border-b pb-2">
              <h3 className="text-xs font-black uppercase text-slate-800 tracking-wider font-sans">
                {lang === "ar"
                  ? "تقديم عرض سعر الشحن"
                  : "Proposer une Cotation Logistique"}
              </h3>
              <button
            ...
            <form onSubmit={handlePublishBid} className="space-y-4 text-xs">
              ...
              <div className="p-3 bg-[#1D9E75]/10 rounded-lg space-y-1">
                <p className="font-extrabold text-slate-950 text-xs">
                  🌍 {translateCity(selectedOffreForBid.depart, lang)} ➔{" "}
                  {translateCity(selectedOffreForBid.arrivee, lang)}
                </p>
                <p className="text-slate-600 text-[11px]">
                  📦{" "}
                  {translateMarchandise(selectedOffreForBid.marchandise, lang)}{" "}
                  · ⚖️ {selectedOffreForBid.poids} t
                </p>
                <p className="text-[11px] font-semibold text-[#187857]">
                  🚛 Matériel exigé :{" "}
                  {translateMoyenType(selectedOffreForBid.moyenExige, lang)}{" "}
                  (Min {selectedOffreForBid.longueurExigee || 12}m)
                </p>
              </div>
              ...
              {moyens.filter((m) => m.transporteurId === currentUser?.id)
                .length === 0 ? (
                <div className="p-4 bg-orange-50 border border-orange-200 text-orange-900 rounded-xl space-y-3.5 text-xs">
                  ...
                  <p className="leading-relaxed">
                    Vous devez déclarer au moins un véhicule lourd dans votre
                    **Espace Flotte** avant de soumettre des offres sur la
                    bourse.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      ...
                      setCurrentTab("transporteur");
                      triggerSystemLog(
                        "Redirigé vers votre espace flotte.",
                        "info",
                      );
                    }}
                ...
                <>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 mb-1">
                      {lang === "ar"
                        ? "اختر المركبة من أسطولك *"
                        : "Choix du camion à utiliser *"}
                    </label>
                    <select
                      value={bidMoyenId}
                      onChange={(e) => {
                    ...
                    >
                      ...
                      {moyens
                        .filter((m) => m.transporteurId === currentUser?.id)
                        .map((m) => (
                          <option key={m.id} value={m.id}>
                            🚚 {m.marque} - [ {m.immatriculation} ] (
                            {m.poidsUtileMax} t)
                          </option>
                        ))}
                    </select>
                    ...
                    {selectedOffreForBid.prixFixe ? (
                      <div className="space-y-2.5 bg-slate-50 p-3 rounded-lg border border-slate-200">
                        <span className="block text-xs font-black text-slate-800 font-mono">
                          📌 PRIX FIXÉ PAR LE DO :{" "}
                          {selectedOffreForBid.prixFixe.toLocaleString()} DZD
                        </span>
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          <span className="text-[10.5px] font-bold text-slate-600">
                            J'accepte d'effectuer le transport à ce tarif fixe
                          </span>
                        </label>
                      </div>
                    ) : (
                      <>
                        ...
                        <span className="text-[10px] text-[#1D9E75] font-semibold mt-1 block">
                          💡 Prix conseillé pour ce trajet : ~
                          {Math.floor(
                            (selectedOffreForBid.prixFixe || 110000) * 0.95,
                          ).toLocaleString()}{" "}
                          DZD
                        </span>
                      </>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-[#6B7280] mb-1">
                      {lang === "ar"
                        ? "توضيحات إضافية"
                        : "Message libre au DO (facultatif)"}
                    </label>
                    <textarea
                      rows={2}
                      placeholder={
                        lang === "ar"
                          ? "مثال: متاح فوراً، سائق مجهز ومجرب..."
                          : "Ex: Chauffeur disponible, matériel bâché..."
                      }
                  ...
                  <label className="flex items-start gap-2 pt-1 border-t cursor-pointer select-none">
                    ...
                    <span className="text-[10.5px] leading-relaxed font-bold text-slate-600">
                      Je certifie que mes documents d'assurance transport de
                      marchandises et documents d'exploitation sont à jour. *
                    </span>
                  </label>
      ...
      <div className="fixed bottom-0 left-0 right-0 h-[64px] bg-white border-t border-[#E5E7EB] flex items-center justify-around px-2 z-40 select-none shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full py-2 transition-all cursor-pointer ${
            currentTab === "accueil"
              ? "text-[#1D9E75]"
              : "text-[#6B7280] hover:text-[#085041]"
          }`}
        >
          <Home
            className={`w-5 h-5 transition-transform ${currentTab === "accueil" ? "scale-110" : ""}`}
          />
        ...
        >
          <Map
            className={`w-5 h-5 transition-transform ${currentTab === "carte" ? "scale-110" : ""}`}
          />
          <span className="text-[10px] uppercase font-bold tracking-wider mt-1 block">
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full py-2 transition-all cursor-pointer ${
            currentTab === "publier"
              ? "text-[#D85A30]"
              : "text-[#6B7280] hover:text-[#D85A30]"
          }`}
        >
          <PlusCircle
            className={`w-5 h-5 transition-transform ${currentTab === "publier" ? "scale-110 stroke-[#D85A30]" : ""}`}
          />
          <span className="text-[10px] uppercase font-bold tracking-wider mt-1 block">
            {lang === "ar" ? "نشر" : "Publier"}
          </span>
        ...
        >
          {(() => {
            const count = !currentUser
              ? 0
              : currentUser.profil === "Donneur d'ordre"
                ? propositions.filter((p) => !p.validee).length
                : currentUser.profil === "Transporteur"
                  ? factures.filter(
                      (f) =>
                        f.transporteurId === currentUser.id &&
                        f.status === ("Facture Transmise" as any),
                    ).length
                  : 1;
            return count > 0 ? (
              <div className="relative">
                <FolderOpen
                  className={`w-5 h-5 transition-transform ${currentTab === "donneur" || currentTab === "transporteur" || currentTab === "commercial" || currentTab === "admin" || currentTab === "login_portal" ? "scale-110" : ""}`}
                />
        ...
        >
          <Menu
            className={`w-5 h-5 transition-transform ${currentTab === "menu" ? "scale-110" : ""}`}
          />
          <span className="text-[10px] uppercase font-bold tracking-wider mt-1 block">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 space-y-4 border border-slate-200 dark:border-slate-800 shadow-2xl relative text-slate-800 dark:text-slate-100 max-h-[90vh] overflow-y-auto animate-slide-up relative z-50">
            ...
            {activeLegalModal === "about" && (
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Digitalisation logistique nationale algérienne
                  </p>
                </div>
                <div className="text-xs leading-relaxed space-y-2.5 font-sans font-medium text-slate-600 dark:text-slate-300">
                  <p>
                    NETLOG est plus qu'un outil d'affrètement : c'est un
                    écosystème conçu pour sécuriser chaque transition
                    industrielle, de la côte méditerranéenne aux infrastructures
                    pétrolières d'Hassi Messaoud.
                  </p>
                  <p>
                    Nous connectons en toute conformité réglementaire les
                    donneurs d'ordres, transporteurs agrées et BVF commerciaux à
                    l'aide d'informations asymétriques assurant le suivi
                    contractuel, le KYC automatisé et la persistance des
                    registres.
                  </p>
                  <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 font-mono text-[9px]">
                    <p className="font-extrabold text-[#1D9E75]">
                      📍 NETLOG ALGERIA INC.
                    </p>
            ...
            {activeLegalModal === "contact" && (
              <div className="space-y-4">
                  ...
                  onSubmit={(e) => {
                    ...
                    showToast(
                      lang === "ar"
                        ...
                        : "📨 Votre message a été transmis avec succès ! Notre support vous contactera sous 24h.",
                      "success",
                    );
                ...
                >
                  <div>
                    <label className="block text-[9.5px] text-slate-500 font-extrabold uppercase mb-1">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
        ...
        >
          ...
          <span>
            {lang === "ar"
              ? "محاكي روابط SMS / E-mail"
              : "Simulateur SMS / E-mail"}
          </span>
          ...
          <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl w-80 sm:w-96 shadow-2xl overflow-hidden max-h-[500px] flex flex-col font-sans select-none animate-slide-up text-slate-800 dark:text-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="leading-tight">
                  <h4 className="text-xs font-black uppercase tracking-wider">
                    Passerelle Télécom Netlog
                  </h4>
                  <p className="text-[10px] text-slate-400 font-medium">
                    Flux d'expédition de liens SMS/E-mail
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-3 bg-rose-50 text-slate-800 text-[10px] font-bold border-b border-rose-100 flex items-center gap-2">
              ...
              <p className="leading-tight">
                Simulez la réception des SMS et E-mails envoyés aux Donneurs
                d'Ordre lors des étapes clés (chargement et livraison). Cliquez
                pour rédiger les réserves !
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3 max-h-[300px]">
              {simulatedNotifs.length === 0 ? (
                <div className="p-8 text-center text-slate-400 dark:text-slate-500 text-xs italic font-medium">
                  Aucun SMS/E-mail émis pour le moment.
                  <br />
                  Déclenchez-en en validant des actions de missions (ex:
                  "Chargement exécuté" ou "J'ai livré") dans l'espace
                  Transporteur !
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span
                          className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${isChargement ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"}`}
                        >
                          {isChargement
                            ? "📋 Chargement réalisé"
                            : "🏁 Livraison effectuée"}
                      ...
                      <div className="space-y-2 mt-1">
                        ...
                        <div className="text-[10px] leading-tight">
                          ...
                          <p className="font-mono text-slate-500 text-[9px] mt-0.5">
                            📞 {notif.destPhone} • 📧 {notif.destEmail}
                          </p>
                          <p className="text-slate-600 dark:text-slate-300 text-[10px] mt-1 font-semibold">
                            📦 Expl:{" "}
                            <span className="text-slate-900 dark:text-slate-150 font-extrabold">
                              {notif.marchandise}
                            </span>{" "}
                            ({notif.trajet})
                          </p>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1 font-mono text-[10px] text-slate-750 dark:text-slate-300">
                          <p className="font-bold text-slate-950 dark:text-white border-b pb-1 mb-1">
                            📟 SMS &amp; EMAIL DEMO TEMPLATE :
                          </p>
                          <p className="italic leading-normal select-text">
                            "{notif.smsText.substring(0, 110)}..."
                          </p>
                        </div>
                        ...
                        <button
                          onClick={() => {
                            // Marquer comme lue
                            const updated = simulatedNotifs.map((n) =>
                            ...
                            const matchedOffre = offres.find(
                              (o) => o.id === notif.offreId,
                            );
                            if (matchedOffre) {
                              const currentVal =
                                notif.type === "chargement"
                                  ? matchedOffre.reservesChargement || ""
                                  : matchedOffre.reservesLivraison ||
                                    matchedOffre.reserves ||
                                    "";
              ...
              <div className="p-2.5 bg-slate-50 dark:bg-[#111827] border-t border-slate-100 dark:border-slate-850 text-center">
                <button
                  onClick={() => {
                    saveSimulatedNotifs([]);
                    triggerSystemLog(
                      "Historique du simulateur de liens vidé.",
                      "info",
                    );
                  }}
      ...
      {notifPortalActive &&
        (() => {
          const matchingOffre = offres.find(
            (o) => o.id === notifPortalActive.offreId,
          );
              ...
              <div className="bg-slate-950 rounded-[40px] border-8 border-slate-800 shadow-2xl w-full max-w-sm overflow-hidden flex flex-col relative aspect-[9/19] h-[780px] max-h-[92vh] text-slate-800">
                ...
                <div className="flex-1 bg-slate-50 overflow-y-auto px-4 py-5 flex flex-col">
                  ...
                  <div className="flex items-center gap-1.5 pb-3 border-b border-slate-200 mb-4">
                    ...
                    <div>
                      <h3 className="text-xs font-black tracking-tight text-emerald-700 leading-none">
                        NETLOG MOBILE PORTAL
                      </h3>
                      <span className="text-[8px] font-bold text-slate-400 tracking-wider">
                        LIEN SÉCURISÉ LOGISTIQUE
                      </span>
                    </div>
                  </div>
                  <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs mb-4">
                    <span
                      className={`text-[8.5px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full inline-block mb-2 ${isChargement ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"}`}
                    >
                      {isChargement
                        ? "📋 Phase de Chargement"
                        : "🏁 Phase de Réception"}
                    ...
                    <h4 className="text-xs font-black text-slate-900 font-mono">
                      Expédition #{matchingOffre.id}
                    </h4>

                    <div className="text-[10px] mt-2 space-y-1 text-slate-600 font-medium">
                      <p>
                        <b>Axe :</b> {matchingOffre.depart} ➔{" "}
                        {matchingOffre.arrivee}
                      </p>
                      <p>
                        <b>Produit :</b> 📦 {matchingOffre.marchandise}
                      </p>
                      <p>
                        <b>Transporteur :</b>{" "}
                        {matchingOffre.transporteurRaisonSociale ||
                          "EURL Ahmed Benzekri Transports"}
                      </p>
                      <p>
                        <b>Tonnage :</b> {matchingOffre.poids} Tonnes
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mt-1">
                    ...
                    <textarea
                      ...
                      placeholder={
                        isChargement
                          ? "Entrez vos réserves de chargement..."
                          : "Entrez vos réserves de livraison (anomalies)..."
                      }
                  ...
                  <div className="bg-amber-50 p-3 rounded-2xl border border-amber-200 mt-4 text-[9.5px] leading-relaxed text-amber-900 font-medium">
                    ⚖️ <b>Avis juridique :</b> En enregistrant ces mentions,
                    elles seront insérées immédiatement de manière
                    contradictoire sur la Lettre de Voiture nationale officielle
                    NETLOG. Cette consignation fait acte de preuve juridique.
                  </div>
                    ...
                    <button
                      onClick={() => {
                        const updatedOffres = offres.map((o) => {
                          if (o.id === matchingOffre.id) {
                            if (isChargement) {
                              return {
                                ...o,
                                reservesChargement: typedReserveText.trim(),
                              };
                            } else {
                              return {
                                ...o,
                                reservesLivraison: typedReserveText.trim(),
                                reserves: typedReserveText.trim(),
                              };
                            }
                          }
                          return o;
                        ...
                        if (notifPortalActive.notifId) {
                          const updatedN = simulatedNotifs.map((n) =>
                            n.id === notifPortalActive.notifId
                              ? { ...n, status: "completed" }
                              : n,
                          );
